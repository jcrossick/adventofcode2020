...
1-2 a: abcaa
`
const testInputPass = `1-1 a: abcd
2-2 a: aabc
`
...
console.log("input legtnh is", input.length);
  const validPasswords = input.filter((p) => {
    const matches = p.password.split("").filter((c) => c == p.letterRequired);
    return matches.length <= p.maxCount && matches.length >= p.minCount;
  });
...
const parseInput = (inputString) => {
  return inputString.split(/\r?\n/).map((s) => {
    return {
      ...
      minCount: parseInt(s.slice(0, s.indexOf("-"))),
      maxCount: parseInt(s.slice(s.indexOf("-") + 1, s.indexOf(" "))),
      password: s.slice(s.indexOf(": ") + 2),
    };
  });
};

run(parseInput(testInputFail));
run(parseInput(testInputPass));
run(parseInput(inputString));
